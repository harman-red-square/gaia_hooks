...
js_changed_files=`git diff $BASELINE $BRANCH --name-only --diff-filter=ACMRT  | grep '\.js$'`
...
css_changed_files=`git diff $BASELINE $BRANCH --name-only --diff-filter=ACMRT  | grep '\.css$' | tr '\n' ' '`
